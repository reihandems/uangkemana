<!-- Sidebar -->
<div class="drawer lg:drawer-open h-full border-r border-almost-white" style="background-color: var(--white);">
    <input id="my-drawer-3" type="checkbox" class="drawer-toggle" />
    <div class="drawer-content flex flex-col m-3">
        <!-- Page content here -->
        <label for="my-drawer-3" class="btn drawer-button lg:hidden" style="background-color: var(--white);">
        <svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" fill="#333333"><path d="M144-264v-72h672v72H144Zm0-180v-72h672v72H144Zm0-180v-72h672v72H144Z"/></svg>
        </label>
        <!-- Sidebar -->
        <div class="sidebar font-bold h-full hidden lg:block" style="padding: 20px 30px; background-color: var(--white);">
            <div class="flex flex-row items-stretch">
                <img src="<?= base_url('assets/img/logo.svg') ?>" alt="" class="me-2">
                <h2 class="self-center" style="color: var(--primary-800)">UangKemana</h2>
            </div>
            <hr class="my-5" style="color: var(--shaded-white)">
            <!-- Open the modal using ID.showModal() method -->
            <button class="btn btn-lg bg-primary-500 text-white w-full" onclick="modal_transaksi.showModal()">+ Tambah Transaksi</button>
            <hr class="my-5" style="color: var(--shaded-white)">
            <ul>
                <li class="list-menu <?= ($menu ?? '') == 'dashboard' ? 'active' : '' ?>">
                    <a href="<?= base_url('/dashboard') ?>">
                        <div class="flex flex-row items-stretch">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                <path d="M10.9951 4.68V8.56C10.9951 8.88127 10.9316 9.19938 10.8084 9.49606C10.6851 9.79275 10.5045 10.0622 10.2769 10.2889C10.0492 10.5156 9.77907 10.6951 9.48188 10.8171C9.18469 10.9392 8.86632 11.0013 8.54506 11H4.68506C4.36466 11.0019 4.04717 10.9392 3.75161 10.8155C3.45606 10.6917 3.18853 10.5096 2.96506 10.28C2.73817 10.055 2.5585 9.78697 2.43658 9.49161C2.31466 9.19625 2.25295 8.87952 2.25506 8.56V4.69C2.25505 4.0446 2.51075 3.42549 2.96618 2.96819C3.42161 2.51088 4.03966 2.25265 4.68506 2.25H8.55506C8.87512 2.25031 9.19195 2.3141 9.48719 2.43769C9.78243 2.56128 10.0502 2.74221 10.2751 2.97C10.5025 3.19298 10.6834 3.45902 10.807 3.75261C10.9306 4.04619 10.9945 4.36145 10.9951 4.68ZM21.7451 4.69V8.56C21.7399 9.2038 21.4825 9.81991 21.0282 10.2761C20.5739 10.7323 19.9588 10.9922 19.3151 11H15.4351C14.7883 10.996 14.1682 10.7416 13.7051 10.29C13.4788 10.0625 13.2996 9.79258 13.1777 9.49572C13.0559 9.19886 12.9938 8.88088 12.9951 8.56V4.69C12.9943 4.36977 13.0575 4.05261 13.1812 3.75722C13.3048 3.46182 13.4864 3.19416 13.7151 2.97C13.9399 2.74221 14.2077 2.56128 14.5029 2.43769C14.7982 2.3141 15.115 2.25031 15.4351 2.25H19.3051C19.9506 2.25523 20.5682 2.51398 21.0246 2.97044C21.4811 3.4269 21.7398 4.04449 21.7451 4.69ZM21.7451 15.44V19.31C21.7399 19.9538 21.4825 20.5699 21.0282 21.0261C20.5739 21.4823 19.9588 21.7422 19.3151 21.75H15.4351C14.7841 21.7566 14.1563 21.5091 13.6851 21.06C13.4579 20.8331 13.2781 20.5634 13.1562 20.2664C13.0343 19.9693 12.9727 19.651 12.9751 19.33V15.46C12.9743 15.1398 13.0375 14.8226 13.1612 14.5272C13.2848 14.2318 13.4664 13.9642 13.6951 13.74C13.9199 13.5122 14.1877 13.3313 14.4829 13.2077C14.7782 13.0841 15.095 13.0203 15.4151 13.02H19.2851C19.9306 13.0252 20.5482 13.284 21.0046 13.7404C21.4611 14.1969 21.7198 14.8145 21.7251 15.46L21.7451 15.44ZM10.9951 15.45V19.32C10.9872 19.9655 10.7259 20.582 10.2676 21.0366C9.80925 21.4912 9.19059 21.7474 8.54506 21.75H4.68506C4.36557 21.7513 4.04899 21.6894 3.75357 21.5677C3.45816 21.4461 3.18975 21.2671 2.96384 21.0412C2.73793 20.8153 2.55899 20.5469 2.43734 20.2515C2.31569 19.9561 2.25373 19.6395 2.25506 19.32V15.45C2.25763 14.8045 2.51385 14.1858 2.96844 13.7275C3.42303 13.2691 4.03957 13.0079 4.68506 13H8.55506C9.20326 13.0066 9.82355 13.2648 10.2851 13.72C10.7411 14.1801 10.9964 14.8021 10.9951 15.45Z"/>
                            </svg>
                            <p class="self-center ml-2">Dashboard</p>
                        </div>
                    </a>
                </li>
                <li class="list-menu <?= ($menu ?? '') == 'transaksi' ? 'active' : '' ?>">
                    <a href="<?= base_url('/transaksi') ?>">
                        <div class="flex flex-row items-stretch">
                            <svg width="24" height="24" viewBox="0 0 40 40" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                            <path d="M31.6667 5H8.33333C6.5 5 5 6.5 5 8.33333V31.6667C5 33.5 6.5 35 8.33333 35H31.6667C33.5 35 35 33.5 35 31.6667V8.33333C35 6.5 33.5 5 31.6667 5ZM11.6667 11.6667H15V15H11.6667V11.6667ZM11.6667 18.3333H15V21.6667H11.6667V18.3333ZM11.6667 25H15V28.3333H11.6667V25ZM28.3333 28.3333H18.3333V25H28.3333V28.3333ZM28.3333 21.6667H18.3333V18.3333H28.3333V21.6667ZM28.3333 15H18.3333V11.6667H28.3333V15Z"/>
                            </svg>
                            <p class="self-center ml-2">Transaksi</p>
                        </div>
                    </a>
                </li>
                <li class="list-menu <?= ($menu ?? '') == 'budget' ? 'active' : '' ?>">
                    <a href="#">
                        <div class="flex flex-row items-stretch">
                            <svg width="24" height="24" viewBox="0 0 40 40" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                            <path d="M33.05 12.5002L29.2667 8.71683C29.3833 8.01683 29.5667 7.36683 29.8 6.80016C29.96 6.42065 30.0237 6.00744 29.9853 5.59737C29.9469 5.18729 29.8076 4.79308 29.5799 4.44988C29.3522 4.10668 29.0431 3.82514 28.6802 3.63035C28.3173 3.43556 27.9119 3.33358 27.5 3.3335C24.7667 3.3335 22.35 4.65016 20.8333 6.66683H12.5C7.43334 6.66683 3.33334 10.7668 3.33334 15.8335C3.33334 20.9002 7.5 35.0002 7.5 35.0002H16.6667V31.6668H20V35.0002H29.1667L31.9667 25.6835L36.6667 24.1168V12.5002H33.05ZM21.6667 15.0002H13.3333V11.6668H21.6667V15.0002ZM26.6667 18.3335C25.75 18.3335 25 17.5835 25 16.6668C25 15.7502 25.75 15.0002 26.6667 15.0002C27.5833 15.0002 28.3333 15.7502 28.3333 16.6668C28.3333 17.5835 27.5833 18.3335 26.6667 18.3335Z"/>
                            </svg>
                            <p class="self-center ml-2">Budget</p>
                        </div>
                    </a>
                </li>
                <li class="list-menu <?= ($menu ?? '') == 'dompet' ? 'active' : '' ?>">
                    <a href="#">
                        <div class="flex flex-row items-stretch">
                            <svg width="24" height="24" viewBox="0 0 40 40" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                            <path d="M28.3333 5H11.6667C7.98333 5 5 7.98333 5 11.6667V28.3333C5 32.0167 7.98333 35 11.6667 35H31.6667C33.5167 35 35 33.5167 35 31.6667V15C35 14.1159 34.6488 13.2681 34.0237 12.643C33.3986 12.0179 32.5507 11.6667 31.6667 11.6667V8.33333C31.6667 7.44928 31.3155 6.60143 30.6904 5.97631C30.0652 5.35119 29.2174 5 28.3333 5ZM28.3333 8.33333V11.6667H11.6667C10.45 11.6667 9.31667 12 8.33333 12.5667V11.6667C8.33333 9.83333 9.83333 8.33333 11.6667 8.33333M25.8333 25.8333C24.45 25.8333 23.3333 24.7167 23.3333 23.3333C23.3333 21.95 24.45 20.8333 25.8333 20.8333C27.2167 20.8333 28.3333 21.95 28.3333 23.3333C28.3333 24.7167 27.2167 25.8333 25.8333 25.8333Z"/>
                            </svg>
                            <p class="self-center ml-2">Dompet</p>
                        </div>
                    </a>
                </li>
                <li class="list-menu <?= ($menu ?? '') == 'settings' ? 'active' : '' ?>">
                    <a href="#">
                        <div class="flex flex-row items-stretch">
                            <svg width="24" height="24" viewBox="0 0 40 40" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                            <path d="M20 25.8335C18.4529 25.8335 16.9692 25.2189 15.8752 24.125C14.7812 23.031 14.1667 21.5473 14.1667 20.0002C14.1667 18.4531 14.7812 16.9693 15.8752 15.8754C16.9692 14.7814 18.4529 14.1668 20 14.1668C21.5471 14.1668 23.0308 14.7814 24.1248 15.8754C25.2188 16.9693 25.8333 18.4531 25.8333 20.0002C25.8333 21.5473 25.2188 23.031 24.1248 24.125C23.0308 25.2189 21.5471 25.8335 20 25.8335ZM32.3833 21.6168C32.45 21.0835 32.5 20.5502 32.5 20.0002C32.5 19.4502 32.45 18.9002 32.3833 18.3335L35.9 15.6168C36.2167 15.3668 36.3 14.9168 36.1 14.5502L32.7667 8.7835C32.5667 8.41683 32.1167 8.26683 31.75 8.41683L27.6 10.0835C26.7333 9.4335 25.8333 8.86683 24.7833 8.45017L24.1667 4.0335C24.1328 3.83719 24.0306 3.65921 23.878 3.53108C23.7255 3.40296 23.5325 3.33296 23.3333 3.3335H16.6667C16.25 3.3335 15.9 3.6335 15.8333 4.0335L15.2167 8.45017C14.1667 8.86683 13.2667 9.4335 12.4 10.0835L8.25 8.41683C7.88333 8.26683 7.43333 8.41683 7.23333 8.7835L3.9 14.5502C3.68333 14.9168 3.78333 15.3668 4.1 15.6168L7.61667 18.3335C7.55 18.9002 7.5 19.4502 7.5 20.0002C7.5 20.5502 7.55 21.0835 7.61667 21.6168L4.1 24.3835C3.78333 24.6335 3.68333 25.0835 3.9 25.4502L7.23333 31.2168C7.43333 31.5835 7.88333 31.7168 8.25 31.5835L12.4 29.9002C13.2667 30.5668 14.1667 31.1335 15.2167 31.5502L15.8333 35.9668C15.9 36.3668 16.25 36.6668 16.6667 36.6668H23.3333C23.75 36.6668 24.1 36.3668 24.1667 35.9668L24.7833 31.5502C25.8333 31.1168 26.7333 30.5668 27.6 29.9002L31.75 31.5835C32.1167 31.7168 32.5667 31.5835 32.7667 31.2168L36.1 25.4502C36.3 25.0835 36.2167 24.6335 35.9 24.3835L32.3833 21.6168Z"/>
                            </svg>
                            <p class="self-center ml-2">Settings</p>
                        </div>
                    </a>
                </li>
            </ul>
            <hr class="my-5" style="color: var(--shaded-white)">
            <a href="<?= base_url('/logout') ?>">
                <button class="btn bg-white text-red-600 border-[#e5e5e5] w-full">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M12 15L15 12M15 12L12 9M15 12H4M9 7.249V7.2C9 6.08 9 5.52 9.218 5.092C9.41 4.715 9.715 4.41 10.092 4.218C10.52 4 11.08 4 12.2 4H16.8C17.92 4 18.48 4 18.907 4.218C19.284 4.41 19.59 4.715 19.782 5.092C20 5.519 20 6.079 20 7.197V16.804C20 17.922 20 18.481 19.782 18.908C19.59 19.2845 19.2837 19.5904 18.907 19.782C18.48 20 17.921 20 16.803 20H12.197C11.079 20 10.519 20 10.092 19.782C9.71569 19.5903 9.40974 19.2843 9.218 18.908C9 18.48 9 17.92 9 16.8V16.75" stroke="#E61919" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
                Logout
                </button>
            </a>
        </div>
        <!-- Sidebar end -->
    </div>
    <div class="drawer-side w-full md:hidden">
        <label for="my-drawer-3" aria-label="close sidebar" class="drawer-overlay"></label>
        <!-- Sidebar -->
        <div class="sidebar font-bold shadow-md h-full" style="padding: 24px 30px; background-color: var(--white);">
            <div class="flex flex-row items-stretch">
                <img src="<?= base_url('assets/img/logo.svg') ?>" alt="" class="me-2">
                <h2 class="self-center" style="color: var(--primary-800)">UangKemana</h2>
            </div>
            <hr class="my-5" style="color: var(--shaded-white)">
            <!-- Open the modal using ID.showModal() method -->
            <button class="btn btn-lg bg-primary-500 text-white w-full" onclick="modal_transaksi.showModal()">+ Tambah Transaksi</button>
            <hr class="my-5" style="color: var(--shaded-white)">
            <ul>
                <li class="list-menu <?= ($menu ?? '') == 'dashboard' ? 'active' : '' ?>">
                    <a href="<?= base_url('/dashboard') ?>">
                        <div class="flex flex-row items-stretch">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                <path d="M10.9951 4.68V8.56C10.9951 8.88127 10.9316 9.19938 10.8084 9.49606C10.6851 9.79275 10.5045 10.0622 10.2769 10.2889C10.0492 10.5156 9.77907 10.6951 9.48188 10.8171C9.18469 10.9392 8.86632 11.0013 8.54506 11H4.68506C4.36466 11.0019 4.04717 10.9392 3.75161 10.8155C3.45606 10.6917 3.18853 10.5096 2.96506 10.28C2.73817 10.055 2.5585 9.78697 2.43658 9.49161C2.31466 9.19625 2.25295 8.87952 2.25506 8.56V4.69C2.25505 4.0446 2.51075 3.42549 2.96618 2.96819C3.42161 2.51088 4.03966 2.25265 4.68506 2.25H8.55506C8.87512 2.25031 9.19195 2.3141 9.48719 2.43769C9.78243 2.56128 10.0502 2.74221 10.2751 2.97C10.5025 3.19298 10.6834 3.45902 10.807 3.75261C10.9306 4.04619 10.9945 4.36145 10.9951 4.68ZM21.7451 4.69V8.56C21.7399 9.2038 21.4825 9.81991 21.0282 10.2761C20.5739 10.7323 19.9588 10.9922 19.3151 11H15.4351C14.7883 10.996 14.1682 10.7416 13.7051 10.29C13.4788 10.0625 13.2996 9.79258 13.1777 9.49572C13.0559 9.19886 12.9938 8.88088 12.9951 8.56V4.69C12.9943 4.36977 13.0575 4.05261 13.1812 3.75722C13.3048 3.46182 13.4864 3.19416 13.7151 2.97C13.9399 2.74221 14.2077 2.56128 14.5029 2.43769C14.7982 2.3141 15.115 2.25031 15.4351 2.25H19.3051C19.9506 2.25523 20.5682 2.51398 21.0246 2.97044C21.4811 3.4269 21.7398 4.04449 21.7451 4.69ZM21.7451 15.44V19.31C21.7399 19.9538 21.4825 20.5699 21.0282 21.0261C20.5739 21.4823 19.9588 21.7422 19.3151 21.75H15.4351C14.7841 21.7566 14.1563 21.5091 13.6851 21.06C13.4579 20.8331 13.2781 20.5634 13.1562 20.2664C13.0343 19.9693 12.9727 19.651 12.9751 19.33V15.46C12.9743 15.1398 13.0375 14.8226 13.1612 14.5272C13.2848 14.2318 13.4664 13.9642 13.6951 13.74C13.9199 13.5122 14.1877 13.3313 14.4829 13.2077C14.7782 13.0841 15.095 13.0203 15.4151 13.02H19.2851C19.9306 13.0252 20.5482 13.284 21.0046 13.7404C21.4611 14.1969 21.7198 14.8145 21.7251 15.46L21.7451 15.44ZM10.9951 15.45V19.32C10.9872 19.9655 10.7259 20.582 10.2676 21.0366C9.80925 21.4912 9.19059 21.7474 8.54506 21.75H4.68506C4.36557 21.7513 4.04899 21.6894 3.75357 21.5677C3.45816 21.4461 3.18975 21.2671 2.96384 21.0412C2.73793 20.8153 2.55899 20.5469 2.43734 20.2515C2.31569 19.9561 2.25373 19.6395 2.25506 19.32V15.45C2.25763 14.8045 2.51385 14.1858 2.96844 13.7275C3.42303 13.2691 4.03957 13.0079 4.68506 13H8.55506C9.20326 13.0066 9.82355 13.2648 10.2851 13.72C10.7411 14.1801 10.9964 14.8021 10.9951 15.45Z"/>
                            </svg>
                            <p class="self-center ml-2">Dashboard</p>
                        </div>
                    </a>
                </li>
                <li class="list-menu <?= ($menu ?? '') == 'transaksi' ? 'active' : '' ?>">
                    <a href="<?= base_url('/transaksi') ?>">
                        <div class="flex flex-row items-stretch">
                            <svg width="24" height="24" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M31.6667 5H8.33333C6.5 5 5 6.5 5 8.33333V31.6667C5 33.5 6.5 35 8.33333 35H31.6667C33.5 35 35 33.5 35 31.6667V8.33333C35 6.5 33.5 5 31.6667 5ZM11.6667 11.6667H15V15H11.6667V11.6667ZM11.6667 18.3333H15V21.6667H11.6667V18.3333ZM11.6667 25H15V28.3333H11.6667V25ZM28.3333 28.3333H18.3333V25H28.3333V28.3333ZM28.3333 21.6667H18.3333V18.3333H28.3333V21.6667ZM28.3333 15H18.3333V11.6667H28.3333V15Z" fill="#AAAAAA"/>
                            </svg>
                            <p class="self-center ml-2">Transaksi</p>
                        </div>
                    </a>
                </li>
                <li class="list-menu <?= ($menu ?? '') == 'budget' ? 'active' : '' ?>">
                    <a href="#">
                        <div class="flex flex-row items-stretch">
                            <svg width="24" height="24" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M33.05 12.5002L29.2667 8.71683C29.3833 8.01683 29.5667 7.36683 29.8 6.80016C29.96 6.42065 30.0237 6.00744 29.9853 5.59737C29.9469 5.18729 29.8076 4.79308 29.5799 4.44988C29.3522 4.10668 29.0431 3.82514 28.6802 3.63035C28.3173 3.43556 27.9119 3.33358 27.5 3.3335C24.7667 3.3335 22.35 4.65016 20.8333 6.66683H12.5C7.43334 6.66683 3.33334 10.7668 3.33334 15.8335C3.33334 20.9002 7.5 35.0002 7.5 35.0002H16.6667V31.6668H20V35.0002H29.1667L31.9667 25.6835L36.6667 24.1168V12.5002H33.05ZM21.6667 15.0002H13.3333V11.6668H21.6667V15.0002ZM26.6667 18.3335C25.75 18.3335 25 17.5835 25 16.6668C25 15.7502 25.75 15.0002 26.6667 15.0002C27.5833 15.0002 28.3333 15.7502 28.3333 16.6668C28.3333 17.5835 27.5833 18.3335 26.6667 18.3335Z" fill="#AAAAAA"/>
                            </svg>
                            <p class="self-center ml-2">Budget</p>
                        </div>
                    </a>
                </li>
                <li class="list-menu <?= ($menu ?? '') == 'dompet' ? 'active' : '' ?>">
                    <a href="#">
                        <div class="flex flex-row items-stretch">
                            <svg width="24" height="24" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M28.3333 5H11.6667C7.98333 5 5 7.98333 5 11.6667V28.3333C5 32.0167 7.98333 35 11.6667 35H31.6667C33.5167 35 35 33.5167 35 31.6667V15C35 14.1159 34.6488 13.2681 34.0237 12.643C33.3986 12.0179 32.5507 11.6667 31.6667 11.6667V8.33333C31.6667 7.44928 31.3155 6.60143 30.6904 5.97631C30.0652 5.35119 29.2174 5 28.3333 5ZM28.3333 8.33333V11.6667H11.6667C10.45 11.6667 9.31667 12 8.33333 12.5667V11.6667C8.33333 9.83333 9.83333 8.33333 11.6667 8.33333M25.8333 25.8333C24.45 25.8333 23.3333 24.7167 23.3333 23.3333C23.3333 21.95 24.45 20.8333 25.8333 20.8333C27.2167 20.8333 28.3333 21.95 28.3333 23.3333C28.3333 24.7167 27.2167 25.8333 25.8333 25.8333Z" fill="#AAAAAA"/>
                            </svg>
                            <p class="self-center ml-2">Dompet</p>
                        </div>
                    </a>
                </li>
                <li class="list-menu <?= ($menu ?? '') == 'settings' ? 'active' : '' ?>">
                    <a href="#">
                        <div class="flex flex-row items-stretch">
                            <svg width="24" height="24" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M20 25.8335C18.4529 25.8335 16.9692 25.2189 15.8752 24.125C14.7812 23.031 14.1667 21.5473 14.1667 20.0002C14.1667 18.4531 14.7812 16.9693 15.8752 15.8754C16.9692 14.7814 18.4529 14.1668 20 14.1668C21.5471 14.1668 23.0308 14.7814 24.1248 15.8754C25.2188 16.9693 25.8333 18.4531 25.8333 20.0002C25.8333 21.5473 25.2188 23.031 24.1248 24.125C23.0308 25.2189 21.5471 25.8335 20 25.8335ZM32.3833 21.6168C32.45 21.0835 32.5 20.5502 32.5 20.0002C32.5 19.4502 32.45 18.9002 32.3833 18.3335L35.9 15.6168C36.2167 15.3668 36.3 14.9168 36.1 14.5502L32.7667 8.7835C32.5667 8.41683 32.1167 8.26683 31.75 8.41683L27.6 10.0835C26.7333 9.4335 25.8333 8.86683 24.7833 8.45017L24.1667 4.0335C24.1328 3.83719 24.0306 3.65921 23.878 3.53108C23.7255 3.40296 23.5325 3.33296 23.3333 3.3335H16.6667C16.25 3.3335 15.9 3.6335 15.8333 4.0335L15.2167 8.45017C14.1667 8.86683 13.2667 9.4335 12.4 10.0835L8.25 8.41683C7.88333 8.26683 7.43333 8.41683 7.23333 8.7835L3.9 14.5502C3.68333 14.9168 3.78333 15.3668 4.1 15.6168L7.61667 18.3335C7.55 18.9002 7.5 19.4502 7.5 20.0002C7.5 20.5502 7.55 21.0835 7.61667 21.6168L4.1 24.3835C3.78333 24.6335 3.68333 25.0835 3.9 25.4502L7.23333 31.2168C7.43333 31.5835 7.88333 31.7168 8.25 31.5835L12.4 29.9002C13.2667 30.5668 14.1667 31.1335 15.2167 31.5502L15.8333 35.9668C15.9 36.3668 16.25 36.6668 16.6667 36.6668H23.3333C23.75 36.6668 24.1 36.3668 24.1667 35.9668L24.7833 31.5502C25.8333 31.1168 26.7333 30.5668 27.6 29.9002L31.75 31.5835C32.1167 31.7168 32.5667 31.5835 32.7667 31.2168L36.1 25.4502C36.3 25.0835 36.2167 24.6335 35.9 24.3835L32.3833 21.6168Z" fill="#AAAAAA"/>
                            </svg>
                            <p class="self-center ml-2">Settings</p>
                        </div>
                    </a>
                </li>
                <hr class="my-5" style="color: var(--shaded-white)">
                <a href="<?= base_url('/logout') ?>">
                    <button class="btn btn-wide bg-white text-red-600 border-[#e5e5e5] w-full">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M12 15L15 12M15 12L12 9M15 12H4M9 7.249V7.2C9 6.08 9 5.52 9.218 5.092C9.41 4.715 9.715 4.41 10.092 4.218C10.52 4 11.08 4 12.2 4H16.8C17.92 4 18.48 4 18.907 4.218C19.284 4.41 19.59 4.715 19.782 5.092C20 5.519 20 6.079 20 7.197V16.804C20 17.922 20 18.481 19.782 18.908C19.59 19.2845 19.2837 19.5904 18.907 19.782C18.48 20 17.921 20 16.803 20H12.197C11.079 20 10.519 20 10.092 19.782C9.71569 19.5903 9.40974 19.2843 9.218 18.908C9 18.48 9 17.92 9 16.8V16.75" stroke="#E61919" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                    Logout
                    </button>
                </a>
            </ul>
        </div>
        <!-- Sidebar end -->
    </div>
</div>
<!-- Sidebar end -->